...
Therefore, these tests were implemented in a simple script which connects to a host to run \textbf{iper3} \ac{tcp} and \ac{udp} tests for 120 seconds each and exporting the results as \ac{json}. To this \ac{json} is added the average latency by executing the \textbf{ping} command with a limit of 50 replies.

For this experiment, a Tailscale client running \textbf{iper3} as a server was placed . Then, with another machine, run the script
...
\centering
  ...
  \caption{Tailscale tunnel Network Throughput (University Library)}
...
\centering
  ...
  \caption{Tailscale tunnel Network Throughput (CP)}
  \label{fig:tcptplibrary}
\end{figure}
