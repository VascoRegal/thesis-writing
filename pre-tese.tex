...
\TitlePage
  ...
  \vspace*{8mm}
  \TEXT{\textbf{Resumo}}
       {Uma rede overlay é um grupo de nós computacionais que comunicam entre si através de um canal lógico, montado sobre uma outra rede. Os nós numa rede overlay, que são geralmente end systems, correm aplicações capazes de realizar operações mais complexas do que simplesmente encaminhar e rotear tráfego. Assim, uma rede overlay aplica regras de encaminhamento e encapsulamento de dados para criar um protocolo de comunicação customizado, correndo sobre as ligações de uma outra rede. Este documento apresenta uma solução para um gestor de redes overlay privadas e seguras baseadas em WireGuard, implementado na rede da Universidade de Aveiro, onde estão presentes regras de firewall altamente restritivas. Esta solução permite que os clientes, nomeadamente os robôs autónomos presentes no Intelligent Robotics and Systems Laboratory, estableçam ligações \ac{p2p} entre si, independetemente da sua localização no campus, encapsulando e encaminhando pacotes WireGuard para um servidor de relay via \ac{http}, usando canais de comunicação préviamente criados. Isto não só ajuda os desenvolvedores, permitindo a interação com os robôs usando máquinas pessoais, mas também possibilita o uso dos robôs em qualquer ponto da Universidade de Aveiro.
       }
\EndTitlePage
\titlepage\ \endtitlepage % empty page
...
\TitlePage
  ...
  \vspace*{8mm}
  \TEXT{\textbf{Abstract}}
       {The \ac{iris} is a research group at the \ac{ua} which develops \ac{ros} based applications where robots communicate with each other through a wireless network. Currently, robots are only able to operate when using the laboratory's internal network, since the university's network employs a set of security mechanisms which forbids direct \ac{p2p} communications from taking place. To enable \ac{p2p} communications in such a scenario, this document presents a self-hosted overlay network manager, based on Tailscale. When using the overlay networks, clients maintain an open \ac{http} connection with a relay server, which is then responsible for delivering encrypted application data encapsulated as \ac{http} to a destination node. Since this encapsulation process implies a degree of communication overhead, the overlay networks' performance is analysed when used in conjunction with \ac{ros} applications, in order to validate the use of the protocol according to \ac{iris}'s projects' requirements. Hence, communicating through the overlays allows the establishment of a \ac{p2p} connection between any two clients, regardless of their physical location in the campus. This not only aids \ac{iris}'s researchers, as they are able to interact directly with the robots through their personal machines, but also makes robot deployments across multiple buildings possible.
       }
\EndTitlePage
\titlepage\ \endtitlepage % empty page
