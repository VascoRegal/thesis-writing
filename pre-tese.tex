...
Any \ac{ros} application is composed of one or more nodes. A \ac{ros} node is perceived as the most basic processing unit, generally serving a specific single purpose. A \ac{ros} application also implies the existence of a master node, responsible for providing other nodes (called slaves) with on-demand information regarding each other. Slave nodes store their information in the master node and retrieve information regarding other slaves when necessary. Hence, a \ac{ros} master will act as a disocvery server, allowing nodes to learn the services running in other application peers and the addresses they should request connections to in order to use them.        

\subsection{Communication}

Having described how nodes in a \ac{ros} application discover themselves, the following paragraphs overview three distinct communication scenarios.  
