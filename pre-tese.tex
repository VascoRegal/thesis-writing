...
This chapter aims to provide some directives on which aspects of the presented solution could be improved or should be more thoroughly analysed, even though the developments present in this document meet the goals outlined in Section \ref{sec:obj}.
...
Although creating stand alone WireGuard overlay networks is possible, this required managing and configuring individual peers, a process raising scalability and ease of administration issues.

Fortunately, Tailscale addresses such flaws, by offering a coordination server capable of orchestrating the creation and configuration of WireGuard peers and respective tunnels. More importantly, Tailscale employs a set of mechanisms and infrastructure which overcome network constraints that would normally prevent the direct establishment of a WireGuard connection.
...
With \ac{iris}'s robots authenticated in the control server and configured with Tailscale interfaces, encrypted communication is possible from anywhere within \ac{ua}'s campus, breaking the current confinement to \ac{iris}'s internal network.
