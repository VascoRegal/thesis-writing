...
As stated before, the data plane in TailScale consists in many \acrshort{p2p} connections - essentially a mesh network. However, it is likely that peers reside behind both NAT mechanisms and Firewall rules, with no open ports. Also, believing nodes will consistently have a consistent static IP is a very naive assumption, specially when considering mobility aspects. To directly connect two peers, no matter what's standing between them, TailScale supports NAT Traversal, a mechanism that makes communication possible without the need for firewall configurations or public-facing open ports ~\cite{tailscalenat2020online}.
