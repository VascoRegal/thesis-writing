...
\acrodef{ua-pt}[UA]{Universidade de Aveiro}
...
\TitlePage
  \HEADER{\BAR\FIG{
         ...
         {2024}
  \TITLE{Vasco Jorge Regal Sousa}
        {Secure Overlay Networks For Peer-To-Peer Robotic Communications \newline
        }
  \TITLE{}
        {Redes Overlay Seguras Para Comunicações Robóticas Ponto a Ponto
        }
\EndTitlePage
...
\fi%
obten\c c\~ao do grau de Mestre
em {Engenharia Informática}
, realizada sob a orienta\c c\~ao cient\'ifica do Doutor {Eurico Farinha Pedrosa}, Professor associado do Departamento de Eletrónica, Telecomunicações e Informática da Universidade de Aveiro e do Doutor André Ventura da Cruz Marnoto Zúquete, Professor catedrático do Departamento de Eletrónica, Telecomunicações e Informática da Universidade de Aveiro}
    \vspace{95mm}


{\TEXT{}{\begin{minipage}[t]{52.5mm}\textsf{Apoio financeiro da FCT e do FSE no âmbito do III Quadro Comunitário de Apoio.}\end{minipage}\hspace*{5mm}\begin{minipage}[t]{52.5mm}\textsf{}\end{minipage}}}
...
\TitlePage
  ...
  \TEXT{\textbf{Palavras Chave}}
        {redes overlay, redes, VPN, Tailscale, robótica, cibersegurança
        }
  \vspace*{10mm}
  \TEXT{\textbf{Resumo}}
       {O \ac{iris} é um grupo de investigação da \ac{ua-pt} onde são desenvolvidos projetos baseados em \ac{ros} onde conjuntos de robôs comunicam entre si através de redes sem fios. Atualmente, os robôs só conseguem comunicar quando conectados à rede interna do laboratório, visto que a rede da universidade aplica um conjunto de mecanismos de segurança que não permitem que ligações \ac{p2p} sejam diretamente estabelecidas entre dois clientes. Para permitir este tipo de ligações na rede da \ac{ua-pt}, este documento apresenta um gestor de redes overlay baseado em Tailscale, hospedado na infraestrutura da universidade. Ao usar estas redes overlay, os clientes criam e mantêm uma ligação \ac{http} com um servidor de relay, que é responsável por receber pacotes encapsulados como \ac{http} e reencaminhar, pelos canais préviamente abertos, aos clientes destino. Já que este encapsulamento introduz um overhead nas comunicações, a performance das redes overlay é também analisada, quando usadas juntamente com ambientes \ac{ros}. Assim, comunicação feita pelas redes overlay permite o estabelecimento de uma ligação \ac{p2p} entre qualquer dois clientes, independetemente da sua localização física dentro do campus. Este processo não só ajuda os investigadores do \ac{iris}, já que estes passam a conseguir interagir com os robôs através das suas máquinas pessoais, mas também permite o uso dos robôs em vários edifícios.
       }
\EndTitlePage
\titlepage\ \endtitlepage % empty page
...
\TitlePage
  \vspace*{55mm}
  \TEXT{\textbf{Keywords}}
        {overlay networks, networking, VPN, Tailscale, robotics, cybersecurity
        }
  \vspace*{10mm}
  \TEXT{\textbf{Abstract}}
       {The \ac{iris} is a research group at the \ac{ua} which develops \ac{ros} based applications where robots communicate with each other through a wireless network. Currently, robots are only able to operate when using the laboratory's internal network, since the university's network employs a set of security mechanisms which forbids direct \ac{p2p} communications from taking place. To enable \ac{p2p} communications in such a scenario, this document presents a self-hosted overlay network manager, based on Tailscale. When using the overlay networks, clients maintain an open \ac{http} connection with a relay server, which is then responsible for delivering encrypted application data encapsulated as \ac{http} to the destination nodes, via the pre-established channels. Since this encapsulation process implies a degree of communication overhead, the overlay networks' performance is analysed when used in conjunction with \ac{ros} environments. Hence, communicating through the overlays allows the establishment of a \ac{p2p} connection between any two clients, regardless of their physical location in the campus. This not only aids \ac{iris}'s researchers, as they are able to interact directly with the robots through their personal machines, but also makes robot deployments across multiple buildings possible.
       }
\EndTitlePage
\titlepage\ \endtitlepage % empty page
...
\TitlePage
  \vspace*{55mm}
  \TEXT{\textbf{reconhecimento do uso de ferramentas IA /recognizing the use of AI tools}}
       {I hereby acknowledge the use of OpenAI's ChatGPT-3.5 (https://chat.openai.com/) to both support the research done in the Background chapter, namely IPSec, OpenVPN and Tailscale, and general troubleshooting and debugging for the deployment on the Virtual Development Environment.
       }
\EndTitlePage
\titlepage\ \endtitlepage % empty page
...
Network security has become a topic of growing interest in any information system, where companies strive to ensure their communications follow principles of integrity and confidentiality while minimising attack vectors that could compromise services and data. With such goals in mind, network topologies are subjected to traffic constraints and security mechanisms to secure their systems. Such is the case in the \ac{ua}, where the network, although covering most of the campus' area, enforces several constraining mechanisms that prevent, for example, the establishment of direct \ac{p2p} connections between two clients without additional firewall configurations.

The \ac{iris} is a research group operating in \ac{ua}'s premises that develops projects using autonomous mobile robots based on \ac{ros}, capable of communicating through a wireless network. Currently, the robots are confined to the laboratory's internal network, since, as mentioned above, \ac{ua}'s highly restrictive network prevents the robots from communicating directly through the \ac{ros} middleware.
...
As the nodes in an overlay network are systems running Internet applications, they are generally capable of performing more computationally demanding operations than simply forwarding and switching traffic. As such, routing traffic through an overlay node allows any application-defined forwarding and data encapsulation operations to be applied to the datagrams, creating a custom communication protocol running on top of the underlay network.
...
Conceptually, a \ac{vpn} is a virtual network that provides functionalities  securing the transmission of data between any two endpoints~\cite{HARMENING2017843}. \acp{vpn} have been a widely used technology for decades, establishing themselves as the backbone to secure communications over the Internet.

The following sections aim to analyse how \ac{vpn} services have evolved, from their traditional design to more recent paradigms, that are able to tackle complexity and scalability issues. Then, it presents an individual overview of notable \ac{vpn} providers and compares their respective advantages, limitations, trade-offs and suitability for the scope of this solution. This state-of-the-art technological survey aims to pick up on similar works~\cite{zuqueteseguranca, berger2006analysis}, which examine established protocols such as OpenVPN and IPSec, while additionally reviewing modern protocols, focusing on WireGuard. This exploration serves as the foundation for understanding which protocol is the most suitable to secure communications for the scenario at hand.
...
Initial experiments were conducted in this environment, in order to perceive the requirements for self-hosting a complete Tailscale solution.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.7\textwidth]{dev.png}
...
DETI & 8.41 & 17.20 & 225.88 & 44.744 & 1.040  \\ \hline
...
After a review of general networking concepts and potential tools and services, WireGuard appeared as the most suitable protocol for encrypted communication.
