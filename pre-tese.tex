...
In this environment, the Headscale instance is hosted within \ac{iris}'s network. This, however, only allows communications from clients residing in the \ac{iris}'s network. For clients to be able to reach the instance from any location in the campus, port forwarding rules were created on a public facing server, which redirects traffic on the Headscale and \ac{stun} ports to the Headscale server. With this configuration, clients within the campus, which can directly reach the public facing proxy machine and can perform authentication.

Since Headscale's host is also an Ubuntu linux environment, installation of this service followed the process described in the development environment (see Section \ref{sec:devhs}). Regarding configuration, however, the \textbf{server\_url} parameter, which points to the endpoint clients should use to authenticate, is now the address of the proxy server.
...
Upon testing this scenario using the two development virtual machines as clients, the established communication is of realy type, which means \ac{DERP} servers are being used to redirect the traffic over \ac{http}, as no other process was able to establish a \ac{p2p} \ac{udp} connection. As the Headscale instance was initially configured to use Tailscale's \ac{DERP} server fleet, which implied the redirection of traffic to a \ac{DERP} server in Germany, the tunnels were achieving an average latency of 452ms, a value unacceptable for the scope of this dissertation. Moreover, as this solution is meant to be used exclusevly by \ac{ua}'s clients, Tailscale's \ac{DERP} fleet was discarded, opting for a self-hosted alternative.

\section{Sample Clients}

\section{UA's Premises Deployment}

\subsection{Public Domain Network Segment}
