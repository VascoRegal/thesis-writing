...
As mentioned above, all identified locations were able to communicate with the client running on \ac{iris} through its tunnel interface, validating the solution's ability to establish Tailscale tunnels from all identified key points within the campus.
...
To evaluate communication performance, four main metrics are collected in each scenario, \textbf{\ac{tcp} throughput}, \textbf{\ac{rtt}}, \textbf{jitter} and \textbf{\ac{udp} packet loss}. Since all connections outside \ac{iris} are relayed, which means traffic is encapsulated over \ac{tcp}, this analysis focus primarily on throughput and \ac{rtt} as indicators.

\subsection{Throughput}

Throughput measures the rate at which information is transmitted through the network.
...
\ac{rtt} times dictate how long a source has to wait to receive a response from its destination. These can be calculated using the \textbf{ping} command, which calculates \ac{rtt} through sent and received \ac{icmp} packets. Experiments run measured the average \ac{rtt} for a total of 50 sent \ac{icmp} packets. Figure \ref{fig:buildlat} presents the average \ac{rtt} measured across the different buildings, while figure \ref{fig:ualats} provides a geographic visualization of this data.

Analysing figure \ref{fig:ualats}, concludes that, although all scenarios present \ac{rtt} values within an acceptable range, there's some discrepancies, not necessarily associated with physical distance. In fact, there are several other factors which could justify these results, namely number of network users in the building at the time of the test and distance to the nearest \ac{ap}.
...
Network jitter measures the variation of packet arrival times. High jitter has an impact on the smoothness of a connection, generally having noticeable negative effects on real time communications and media streaming \cite{claypool1999effects}. Figure \ref{fig:uajitter} showcases the gathered jitter results.
...
Although the scripts presented in Chapter \ref{ch:auto} provide a configurable automated procedure to deploy both an Headscale server and Tailscale clients built with custom binaries, this logic could easily be ported to more adequate \ac{iac} frameworks. In fact, the steps taken in said scripts are easily implemented as, for example, Ansible \footnote{Ansible, https://www.ansible.com/} playbooks. This would allow a team of robots to be defined as an Ansible inventory, creating a true one-touch deployment, not requiring the scripts to be run manually in each individual peer.

\subsubsection{Configuring Tailscale's \acp{acl}}

An \ac{acl} or Access Control List is a Tailscale functionality capable of adding an extra security layer to overlay networks by defining policies allowing or denying traffic between nodes. These policies can be applied to Tailscale users, tags, and client groups. Ideally, when deploying a team of robots, these should be grouped and traffic restricted to that specific group, creating an additional logical segmentation to the network, furthering the confidentiality and privacy of the solution.

Regarding implementation of such an \ac{acl} system could easily be achieved built on top of the previous point. In other words, a one touch deployment of a team of robots through \ac{iac} software would allow the creation of Tailscale groups, associating a team's clients to that group and, finally, applying the \ac{acl}.
...
To confine this solution in \ac{ua}'s private network, Headscale, an open-source implementation of Tailscale's coordination server, was used alongside necessary additional services, namely a \ac{DERP} and a \ac{stun} server. Regarding clients, instead of using Tailscale's client software, which does not support the use of self-signed certificates, a fork was created implementing a command line option to accept such certificates, useful for the scenario at hand.
...
Overall, this project delivers automated procedures to configure and deploy a self-hosted secure overlay network manager requiring minimal client configuration, accomplishing the goals outlined in Section \ref{sec:obj}.

With \ac{iris}'s robots authenticated in the self-hosted Headscale control server and, consequently, configured with Tailscale interfaces, encrypted communication is possible from anywhere within \ac{ua}'s campus, successfully enabling robot operation not confined to \ac{iris}'s internal network.
