...
Tailscale also successfully supports procedures to overcome the problems described in the introduction of this section. Regarding \acrshort{nat}, Tailscale uses ICE and STUN based techniques [TODO: describe NAT-traversal on tailscale].

Additionally, and since some networks block UDP entirely, which renders these traversal techniques useless, Tailscale also provides a network of \acrfull{DERP} servers, which are responsible to relay packets over TCP. A Tailscale client is able to forward its encrypted packets to one of such \acrshort{DERP} servers. Since a client's private key never actually leaves its node, \acrshort{DERP} servers can't decrypt the traffic being relayed, performing only redirection of already encrypted packets. These relay servers are distributed geographically in a topology in which the added latency is nearly unnoticed.
...
The tasks encompassing the aforementioned described phases can be summed up in a Gantt diagram, presented in figure \ref{fig:gantt}.
